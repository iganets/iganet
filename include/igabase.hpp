...
/// @{
template <typename, typename, typename = void> class IgABase2;

template <detail::HasAsTensor... Inputs, detail::HasAsTensor... Outputs,
          detail::HasAsTensor... CollPts>
class IgABase2<std::tuple<Inputs...>, std::tuple<Outputs...>,
               std::tuple<CollPts...>> {
public:
  /// @brief Value type
  using value_type =
  ...
  collPts(enum collPts collPtsType, std::index_sequence<Is...>) const {
    ...
    case collPts::greville:
      ...
      ((std::get<Is>(collPts.first) =
            std::get<index>(collPts_).template space<Is>().greville(
                /* interior */ false)),
       ...);

      // Get Greville abscissae at the domain
      ((std::get<Is>(collPts.second) =
            std::get<index>(collPts_).template boundary<Is>().greville()),
       ...);
      break;
    ...
    case collPts::greville_ref1:
      // Get Greville abscissae inside the domain and at the boundary
      ((std::get<Is>(collPts.first) = std::get<index>(collPts_)
                                          .template space<Is>()
                                          .clone()
                                          .uniform_refine()
                                          .greville(
      ...
      // Get Greville abscissae at the domain
      ((std::get<Is>(collPts.second) = std::get<index>(collPts_)
...
public:
  ...
  collPts(enum collPts collPts) const {
    if constexpr (std::tuple_element_t<index, inputs_type>::nspaces() == 1)
      ...
      case collPts::greville:
        return {
            std::get<index>(collPts_).space().greville(/* interior */ false),
      ...
      case collPts::greville_ref1:
        return {
            std::get<index>(collPts_).space().clone().uniform_refine().greville(
                /* interior */ false),
            std::get<index>(collPts_)
                .boundary()
                .clone()
                .uniform_refine()
                .greville()};
      ...
      return collPts(
          collPts, std::make_index_sequence<
                       std::tuple_element_t<index, inputs_type>::nspaces()>{});
  }
};
...
public:
  ...
  IgABase2(const std::array<int64_t, NumCoeffsInputs> &ncoeffsInputs,
           ...
           enum init init = init::greville,
           iganet::Options<value_type> options = iganet::Options<value_type>{})
      : IgABase2(std::tuple{ncoeffsInputs}, std::tuple{ncoeffsOutputs}, init,
                 options) {}
...
private:
  ...
  collPts(enum collPts collPtsType, std::index_sequence<Is...>) const {
    ...
    case collPts::greville:
      // Get Greville abscissae inside the domain and at the boundary
      ((std::get<Is>(collPts.first) =
            std::get<index>(outputs_).template space<Is>().greville(
                /* interior */ false)),
       ...);

      // Get Greville abscissae at the domain
      ((std::get<Is>(collPts.second) =
            std::get<index>(outputs_).template boundary<Is>().greville()),
       ...);
      break;
    ...
    case collPts::greville_ref1:
      // Get Greville abscissae inside the domain and at the boundary
      ((std::get<Is>(collPts.first) = std::get<index>(outputs_)
                                          .template space<Is>()
                                          .clone()
                                          .uniform_refine()
                                          .greville(
      ...
      // Get Greville abscissae at the domain
      ((std::get<Is>(collPts.second) = std::get<index>(outputs_)
                                           .template boundary<Is>()
                                           .clone()
                                           .uniform_refine()
                                           .greville()),
...
public:
  ...
  collPts(enum collPts collPts) const {
    if constexpr (std::tuple_element_t<index, outputs_type>::nspaces() == 1)
      ...
      case collPts::greville:
        return {
            std::get<index>(outputs_).space().greville(/* interior */ false),
      ...
      case collPts::greville_ref1:
        return {
            std::get<index>(outputs_).space().clone().uniform_refine().greville(
                /* interior */ false),
            std::get<index>(outputs_)
                .boundary()
                .clone()
                .uniform_refine()
                .greville()};
      ...
      return collPts(
          collPts, std::make_index_sequence<
                       std::tuple_element_t<index, outputs_type>::nspaces()>{});
  }
};
/// @}
