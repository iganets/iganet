...
public:
  ...
  template <typename Spline> requires SplineType<Spline> auto &next(Spline &obj) const {
...
public:
  /// Default constructor
  RectangleCreator()
    : x0min_(0.0), x0max_(0.1), x1min_(0.9), x1max_(1.0),
      y0min_(0.0), y0max_(0.1), y1min_(0.9), y1max_(1.0) {
    std::srand(std::time(0));
  }
...
public:
  ...
  CuboidCreator()
    : x0min_(0.0), x0max_(0.1), x1min_(0.9), x1max_(1.0),
      y0min_(0.0), y0max_(0.1), y1min_(0.9), y1max_(1.0),
      z0min_(0.0), z0max_(0.1), z1min_(0.9), z1max_(1.0) {      
    std::srand(std::time(0));
  }

  /// Bounds constructor
  CuboidCreator(const T &x0min, const T &x0max, const T &x1min,
                   const T &x1max, const T &y0min, const T &y0max,
                   const T &y1min, const T &y1max, const T &z0min,
                   const T &z0max, const T &z1min, const T &z1max)
      : x0min_(x0min), x0max_(x0max), x1min_(x1min), x1max_(x1max),
        y0min_(y0min), y0max_(y0max), y1min_(y1min), y1max_(y1max),
  ...
  template <typename Spline> requires SplineType<Spline> auto &next(Spline &obj) const {
    ...
    obj.transform([xmin, xmax, ymin, ymax, zmin, zmax](const std::array<T, 3> X) {
...
private:
  T x0min_, x0max_, x1min_, x1max_, y0min_, y0max_, y1min_, y1max_, z0min_, z0max_, z1min_, z1max_;
};  
