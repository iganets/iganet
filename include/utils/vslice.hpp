...
inline auto VSlice(const utils::TensorArray<N> &index,
    ...
    auto partial_prod_ = []<typename T>(T array,
        ...
        auto vslice_summand_ = [&](std::size_t k)
                               {
                                 if (k == N-1) {
                                   ...
                                   if constexpr (N == 2) {
                                       return index[0]
                                         .repeat_interleave(dist[0])
                                         .repeat_interleave(dist[1]) +
                                     ...
                                     return index[0]
                                             .repeat_interleave(dist[0])
                                             .repeat_interleave(partial_prod_(dist, 1, N-1)) +
                                             torch::linspace(start_offset[0], stop_offset[0] - 1, dist[0],
                                                             index[0].options())
                                       .repeat(index[0].numel())
      ...
      // Lambda expression to evaluate the k-th summand of the vslice
        auto vslice_summand_ = [&](std::size_t k)
                               {                                 
                                 if (k == N-1) {
                                   return (index[k]
                                           .repeat(partial_prod_(dist, 0, k)) +
                                           torch::linspace(start_offset[k], stop_offset[k] - 1, dist[k],
                                                           index[0].options())
                                           .repeat_interleave(index[0].numel() * partial_prod_(dist, 0, k-1))) *
                                     partial_prod_(leading_dim, 0, k-1);
                                 }
                                 else if (k == 0) {
                                   if constexpr (N == 2) {
