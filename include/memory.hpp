...
namespace iganet {
  ...
  public:
    ...
    MemoryDebugger()
      : counter_(0), bytes_(0), init_(std::chrono::high_resolution_clock::now())
    {}
    ...
    void add(const std::string& name, const T& obj)
    {
      counter_++;
      bytes_  += sizeof(obj);
      objects_.insert(std::pair<std::chrono::high_resolution_clock::time_point,
                      MemoryObject>(std::chrono::high_resolution_clock::now(),
