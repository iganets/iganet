...
### 1.2 Create a new session
   ...
   -   The `models` list describes the models supported by the server. 
       ...
       Moreover, each entry in the `inputs` and `outputs` list has the form
       ```
       { "name"        : <string>,
         "description" : <string>,
         "type"        : <iotype descriptor> }
       ```
       whereby the `iotype descriptor` must be one of the following

       | `type`        | description   |
       |--------------:|:--------------|
       | `scalar`      | scalar value (e.g., error or drag/lift coefficient) |
       | `scalarfield` | scalar field (e.g., pressure) |
       | `vectorfield` | vector field (e.g., velocity) |
       | `scalarfield_boundary` | scalar field at the boundary (e.g., pressure boundary conditions) |
       | `vectorfield_boundary` | vector field at the boundary (e.g., velocity) |
