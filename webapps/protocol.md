...
### 1.2 Create a new session
   ...
   "status"  : 0
   "data"    : { "id" : session-id,
                 "token" : auto-generated session token }
   ```
   -   The session `token` is generated by the server and serves as authentification mechanism for (re-)connecting to or removing a running session.
