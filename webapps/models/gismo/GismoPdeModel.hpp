    ...
    public:
        ...
        : GismoModel<T>()
      {
        if constexpr(d == 1) {
          ...
          geo_.addPatch(gsTensorBSpline<3, T>(give(KV0), give(KV1), give(KV2), give(C)));
          ...
          geo_.addPatch(gsTensorBSpline<4, T>(give(KV0), give(KV1), give(KV2), give(KV3), give(C)));
      ...
      nlohmann::json to_json(const std::string &component,
        ...
        if (attribute != "") {
          ...
          if (attribute == "degrees") {            
            ...
            else if (auto bspline = dynamic_cast<const gsTensorBSpline<d, T>*>(&geo_.patch(0)))              
              for (std::size_t i = 0; i < bspline->parDim(); ++i)
                json["ncoeffs"].push_back(bspline->basis().size(i));
            else
              return "{ INVALID REQUEST }";
