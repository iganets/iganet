...
namespace iganet {
  ...
  inline void mobiusTransform(const gsAsConstVector<T>& c,
    ...
    jac(0,0) = (alpha-1)*(xi_denominator - (2*alpha-1)*s)/(xi_denominator*xi_denominator);
    jac(0,1) = (alpha_1-alpha_2)*s*(xi_denominator-(alpha-1)*(2*s-1))/(xi_denominator*xi_denominator);
    jac(1,0) = (beta_1-beta_2)*t*(eta_denominator-(beta-1)*(2*t-1))/(eta_denominator*eta_denominator);
    jac(1,1) = (beta-1)*(eta_denominator-(2*beta-1)*t)/(eta_denominator*eta_denominator);
  }
  ...
  public:
    ...
    explicit gsObjFuncSurface(const gismo::gsMultiPatch<T> &patches,
      ...
      //    gsComposedBasis<T> cbasis(mobiusDomain, tbasis); // basis(u,v) = basis(sigma(xi,eta)) -> deriv will give dphi/dxi, dphi/deta
      //
    ...
    T evalObj(const gismo::gsAsConstVector<T> &coefsM) const override {
      ...
      //  gsComposedGeometry<real_t> cgeom(mobiusDomain, tgeom); // G(u,v) = G(sigma(xi,eta))  -> deriv will give dG/dxi, dG/deta
      ...
      //  T val = m_evaluator.integral(m_integration);
      ...
      //
      //    // map surface
      //    auto basisDerivs = m_mp.basis(0).collocationMatrixWithDeriv(m_mp.basis(0), xieta);
      //
