...
TEST_F(FunctionSpaceTest, S1_geoDim1_degrees2)
{
  ...
  { // Interior
    ...
    auto igrad = functionspace.template igrad<functionspace::interior, false>(functionspace, xi);
    ...
    auto ijac = functionspace.template ijac<functionspace::interior, false>(functionspace, xi);
    ...
    auto ihess = functionspace.template ihess<functionspace::interior, false>(functionspace, xi);
  ...
  { // Boundary
    ...
    for (std::size_t i=0; i<std::get<0>(grad).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*std::get<side::left -1>(grad)[i], torch::zeros({1}, options)));
      EXPECT_TRUE(torch::equal(*std::get<side::right-1>(grad)[i], torch::zeros({1}, options)));
    }

    /// Evaluation of gradient (in physical domain)
    auto igrad = functionspace.template igrad<functionspace::boundary, false>(functionspace, xi);

    for (std::size_t i=0; i<std::get<0>(igrad).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*std::get<side::left -1>(igrad)[i], torch::zeros({1}, options)));
      EXPECT_TRUE(torch::equal(*std::get<side::right-1>(igrad)[i], torch::zeros({1}, options)));
    }
    ...
    for (std::size_t i=0; i<std::get<0>(jac).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*std::get<side::left -1>(jac)[i], torch::zeros({1}, options)));
      EXPECT_TRUE(torch::equal(*std::get<side::right-1>(jac)[i], torch::zeros({1}, options)));
    }

    /// Evaluation of Jacobian (in physical domain)
    auto ijac = functionspace.template ijac<functionspace::boundary, false>(functionspace, xi);

    for (std::size_t i=0; i<std::get<0>(ijac).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*std::get<side::left -1>(ijac)[i], torch::zeros({1}, options)));
      EXPECT_TRUE(torch::equal(*std::get<side::right-1>(ijac)[i], torch::zeros({1}, options)));
    }
    ...
    for (std::size_t i=0; i<std::get<0>(hess).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*std::get<side::left -1>(hess)[i], torch::zeros({1}, options)));
      EXPECT_TRUE(torch::equal(*std::get<side::right-1>(hess)[i], torch::zeros({1}, options)));
    }

    /// Evaluation of Hessian (in physical domain)
    auto ihess = functionspace.template ihess<functionspace::boundary, false>(functionspace, xi);

    for (std::size_t i=0; i<std::get<0>(ihess).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*std::get<side::left -1>(ihess)[i], torch::zeros({1}, options)));
      EXPECT_TRUE(torch::equal(*std::get<side::right-1>(ihess)[i], torch::zeros({1}, options)));
    }
  }
}
...
TEST_F(FunctionSpaceTest, S2_geoDim1_degrees23)
{
  ...
  { // Interior
    ...
    /// Evaluation of gradient (in physical domain)
    auto igrad = functionspace.template igrad<functionspace::interior, false>(S2_geometry, xi);
    ...
    /// Evaluation of Jacobian (in physical domain)
    auto ijac = functionspace.template ijac<functionspace::interior, false>(S2_geometry, xi);
    ...
    /// Evaluation of Hessian (in physical domain)
    auto ihess = functionspace.template ihess<functionspace::interior, false>(S2_geometry, xi);
  ...
  { // Boundary
    ...
    for (std::size_t i=0; i<std::get<0>(grad).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(grad)[i]),
                               *(bspline_bdrNS.grad<false>(std::get<side::north-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(grad)[i]),
                               *(bspline_bdrNS.grad<false>(std::get<side::south-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(grad)[i]),
                               *(bspline_bdrEW.grad<false>(std::get<side::east-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(grad)[i]),
    ...
    for (std::size_t i=0; i<std::get<0>(igrad).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(igrad)[i]),
                               *(bspline_bdrNS.igrad<false>(bspline_bdrNS, std::get<side::north-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(igrad)[i]),
                               *(bspline_bdrNS.igrad<false>(bspline_bdrNS, std::get<side::south-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(igrad)[i]),
                               *(bspline_bdrEW.igrad<false>(bspline_bdrEW, std::get<side::east-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(igrad)[i]),
                               *(bspline_bdrEW.igrad<false>(bspline_bdrEW, std::get<side::west-1>(xi))[i])));
    }
    ...
    for (std::size_t i=0; i<std::get<0>(jac).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(jac)[i]),
                               *(bspline_bdrNS.jac<false>(std::get<side::north-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(jac)[i]),
                               *(bspline_bdrNS.jac<false>(std::get<side::south-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(jac)[i]),
                               *(bspline_bdrEW.jac<false>(std::get<side::east-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(jac)[i]),
    ...
    for (std::size_t i=0; i<std::get<0>(ijac).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(ijac)[i]),
                               *(bspline_bdrNS.ijac<false>(bspline_bdrNS, std::get<side::north-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(ijac)[i]),
                               *(bspline_bdrNS.ijac<false>(bspline_bdrNS, std::get<side::south-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(ijac)[i]),
                               *(bspline_bdrEW.ijac<false>(bspline_bdrEW, std::get<side::east-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(ijac)[i]),
                               *(bspline_bdrEW.ijac<false>(bspline_bdrEW, std::get<side::west-1>(xi))[i])));
    }
    ...
    for (std::size_t i=0; i<std::get<0>(hess).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(hess)[i]),
                               *(bspline_bdrNS.hess<false>(std::get<side::north-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(hess)[i]),
                               *(bspline_bdrNS.hess<false>(std::get<side::south-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(hess)[i]),
                               *(bspline_bdrEW.hess<false>(std::get<side::east-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(hess)[i]),
    ...
    for (std::size_t i=0; i<std::get<0>(ihess).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(ihess)[i]),
                               *(bspline_bdrNS.ihess<false>(bspline_bdrNS, std::get<side::north-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(ihess)[i]),
                               *(bspline_bdrNS.ihess<false>(bspline_bdrNS, std::get<side::south-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(ihess)[i]),
                               *(bspline_bdrEW.ihess<false>(bspline_bdrEW, std::get<side::east-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(ihess)[i]),
                               *(bspline_bdrEW.ihess<false>(bspline_bdrEW, std::get<side::west-1>(xi))[i])));
    }
  }
}
...
TEST_F(FunctionSpaceTest, S3_geoDim1_degrees234)
{
  ...
  { // Interior
    ...
    /// Evaluation of gradient (in physical domain)
    auto igrad = functionspace.template igrad<functionspace::interior, false>(S3_geometry, xi);
    auto igrad_ref = bspline.igrad(geometry, xi);
    ...
    /// Evaluation of Jacobian (in physical domain)
    auto ijac = functionspace.template ijac<functionspace::interior, false>(S3_geometry, xi);
    auto ijac_ref = bspline.ijac(geometry, xi);
    ...
    /// Evaluation of Hessian (in physical domain)
    auto ihess = functionspace.template ihess<functionspace::interior, false>(S3_geometry, xi);
    auto ihess_ref = bspline.ihess(geometry, xi);
  ...
  { // Boundary
    ...
    iganet::UniformBSpline<real_t, 3, 2, 4> geometry_bdrNS({5, 7}, iganet::init::greville, options);
    iganet::UniformBSpline<real_t, 3, 3, 4> geometry_bdrEW({4, 7}, iganet::init::greville, options);
    iganet::UniformBSpline<real_t, 3, 2, 3> geometry_bdrFB({5, 4}, iganet::init::greville, options);
    ...
    for (std::size_t i=0; i<std::get<0>(grad).entries(); ++i) {
      ...
      EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(grad)[i]),
                               *(bspline_bdrEW.grad<false>(std::get<side::west-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::front-1>(grad)[i]),
                               *(bspline_bdrFB.grad<false>(std::get<side::front-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(grad)[i]),
                               *(bspline_bdrFB.grad<false>(std::get<side::back-1>(xi))[i])));
    }

    /// Evaluation of gradient (in physical domain)
    auto igrad = functionspace.template igrad<functionspace::boundary, false>(S3_geometry, xi);

    for (std::size_t i=0; i<std::get<0>(igrad).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(igrad)[i]),
                               *(bspline_bdrNS.igrad<false>(geometry_bdrNS, std::get<side::north-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(igrad)[i]),
                               *(bspline_bdrNS.igrad<false>(geometry_bdrNS, std::get<side::south-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(igrad)[i]),
                               *(bspline_bdrEW.igrad<false>(geometry_bdrEW, std::get<side::east-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(igrad)[i]),
                               *(bspline_bdrEW.igrad<false>(geometry_bdrEW, std::get<side::west-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::front-1>(igrad)[i]),
                               *(bspline_bdrFB.igrad<false>(geometry_bdrFB, std::get<side::front-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(igrad)[i]),
                               *(bspline_bdrFB.igrad<false>(geometry_bdrFB, std::get<side::back-1>(xi))[i])));
    }
    ...
    for (std::size_t i=0; i<std::get<0>(jac).entries(); ++i) {
      ...
      EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(jac)[i]),
                               *(bspline_bdrEW.jac<false>(std::get<side::west-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::front-1>(jac)[i]),
                               *(bspline_bdrFB.jac<false>(std::get<side::front-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(jac)[i]),
                               *(bspline_bdrFB.jac<false>(std::get<side::back-1>(xi))[i])));
    }

    /// Evaluation of Jacobian (in physical domain)
    auto ijac = functionspace.template ijac<functionspace::boundary, false>(S3_geometry, xi);

    for (std::size_t i=0; i<std::get<0>(ijac).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(ijac)[i]),
                               *(bspline_bdrNS.ijac<false>(geometry_bdrNS, std::get<side::north-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(ijac)[i]),
                               *(bspline_bdrNS.ijac<false>(geometry_bdrNS, std::get<side::south-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(ijac)[i]),
                               *(bspline_bdrEW.ijac<false>(geometry_bdrEW, std::get<side::east-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(ijac)[i]),
                               *(bspline_bdrEW.ijac<false>(geometry_bdrEW, std::get<side::west-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::front-1>(ijac)[i]),
                               *(bspline_bdrFB.ijac<false>(geometry_bdrFB, std::get<side::front-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(ijac)[i]),
                               *(bspline_bdrFB.ijac<false>(geometry_bdrFB, std::get<side::back-1>(xi))[i])));
    }
    ...
    for (std::size_t i=0; i<std::get<0>(hess).entries(); ++i) {
      ...
      EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(hess)[i]),
                               *(bspline_bdrEW.hess<false>(std::get<side::west-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::front-1>(hess)[i]),
                               *(bspline_bdrFB.hess<false>(std::get<side::front-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(hess)[i]),
                               *(bspline_bdrFB.hess<false>(std::get<side::back-1>(xi))[i])));
    }

    /// Evaluation of Hessian (in physical domain)
    auto ihess = functionspace.template ihess<functionspace::boundary>(S3_geometry, xi);

    for (std::size_t i=0; i<std::get<0>(ihess).entries(); ++i) {
      EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(ihess)[i]),
                               *(bspline_bdrNS.ihess<false>(geometry_bdrNS, std::get<side::north-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(ihess)[i]),
                               *(bspline_bdrNS.ihess<false>(geometry_bdrNS, std::get<side::south-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(ihess)[i]),
                               *(bspline_bdrEW.ihess<false>(geometry_bdrEW, std::get<side::east-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(ihess)[i]),
                               *(bspline_bdrEW.ihess<false>(geometry_bdrEW, std::get<side::west-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::front-1>(ihess)[i]),
                               *(bspline_bdrFB.ihess<false>(geometry_bdrFB, std::get<side::front-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(ihess)[i]),
                               *(bspline_bdrFB.ihess<false>(geometry_bdrFB, std::get<side::back-1>(xi))[i])));
    }
  }
}
...
TEST_F(FunctionSpaceTest, S4_geoDim1_degrees2341)
{
  ...
  { // Interior
    ...
    EXPECT_TRUE(torch::equal(*(functionspace.eval<functionspace::interior, deriv::dt, false>(xi)[0]),
                             *(bspline.eval<deriv::dt, false>(xi)[0])));

    EXPECT_TRUE(torch::equal(*(functionspace.eval<functionspace::interior, deriv::dt^2, false>(xi)[0]),
    ...
    EXPECT_TRUE(torch::equal(*(functionspace.eval<functionspace::interior, deriv::dx+deriv::dt, false>(xi)[0]),
                             *(bspline.eval<deriv::dx+deriv::dt, false>(xi)[0])));

    EXPECT_TRUE(torch::equal(*(functionspace.eval<functionspace::interior, deriv::dy+deriv::dz, false>(xi)[0]),
                             *(bspline.eval<deriv::dy+deriv::dz, false>(xi)[0])));

    EXPECT_TRUE(torch::equal(*(functionspace.eval<functionspace::interior, deriv::dy+deriv::dt, false>(xi)[0]),
                             *(bspline.eval<deriv::dy+deriv::dt, false>(xi)[0])));

    EXPECT_TRUE(torch::equal(*(functionspace.eval<functionspace::interior, deriv::dz+deriv::dt, false>(xi)[0]),
    ...
    basfunc = functionspace.template eval_basfunc<functionspace::interior, deriv::dt, false>(xi, knot_indices);
    EXPECT_TRUE(torch::equal(*(functionspace.eval_from_precomputed<functionspace::interior>(basfunc, coeff_indices, xi[0].numel(), xi[0].sizes())[0]),
                             *(bspline.eval<deriv::dt, false>(xi)[0])));

    basfunc = functionspace.template eval_basfunc<functionspace::interior, deriv::dt^2, false>(xi, knot_indices);
    EXPECT_TRUE(torch::equal(*(functionspace.eval_from_precomputed<functionspace::interior>(basfunc, coeff_indices, xi[0].numel(), xi[0].sizes())[0]),
                             *(bspline.eval<deriv::dt^2, false>(xi)[0])));
    ...
    basfunc = functionspace.template eval_basfunc<functionspace::interior, deriv::dx+deriv::dt, false>(xi, knot_indices);
    EXPECT_TRUE(torch::equal(*(functionspace.eval_from_precomputed<functionspace::interior>(basfunc, coeff_indices, xi[0].numel(), xi[0].sizes())[0]),
                             *(bspline.eval<deriv::dx+deriv::dt, false>(xi)[0])));
    ...
    basfunc = functionspace.template eval_basfunc<functionspace::interior, deriv::dy+deriv::dt, false>(xi, knot_indices);
    EXPECT_TRUE(torch::equal(*(functionspace.eval_from_precomputed<functionspace::interior>(basfunc, coeff_indices, xi[0].numel(), xi[0].sizes())[0]),
                             *(bspline.eval<deriv::dy+deriv::dt, false>(xi)[0])));

    basfunc = functionspace.template eval_basfunc<functionspace::interior, deriv::dz+deriv::dt, false>(xi, knot_indices);
    EXPECT_TRUE(torch::equal(*(functionspace.eval_from_precomputed<functionspace::interior>(basfunc, coeff_indices, xi[0].numel(), xi[0].sizes())[0]),
                             *(bspline.eval<deriv::dz+deriv::dt, false>(xi)[0])));
    ...
    /// Evaluation of gradient (in physical domain)
    auto igrad = functionspace.template igrad<functionspace::interior, false>(S4_geometry, xi);
    auto igrad_ref = bspline.igrad(geometry, xi);
    ...
    /// Evaluation of Jacobian (in physical domain)
    auto ijac = functionspace.template ijac<functionspace::interior, false>(S4_geometry, xi);
    auto ijac_ref = bspline.ijac(geometry, xi);
    ...
    /// Evaluation of Hessian (in physical domain)
    auto ihess = functionspace.template ihess<functionspace::interior, false>(S4_geometry, xi);
    auto ihess_ref = bspline.ihess(geometry, xi);
  ...
  { // Boundary
    ...
    iganet::UniformBSpline<real_t, 1, 2, 4, 1>  bspline_bdrNS({5, 7, 3}, iganet::init::greville, options);
    iganet::UniformBSpline<real_t, 1, 3, 4, 1>  bspline_bdrEW({4, 7, 3}, iganet::init::greville, options);
    iganet::UniformBSpline<real_t, 1, 2, 3, 1>  bspline_bdrFB({5, 4, 3}, iganet::init::greville, options);
    iganet::UniformBSpline<real_t, 1, 2, 3, 4>  bspline_bdrSE({5, 4, 7}, iganet::init::greville, options);
    iganet::UniformBSpline<real_t, 4, 2, 4, 1> geometry_bdrNS({5, 7, 3}, iganet::init::greville, options);
    iganet::UniformBSpline<real_t, 4, 3, 4, 1> geometry_bdrEW({4, 7, 3}, iganet::init::greville, options);
    iganet::UniformBSpline<real_t, 4, 2, 3, 1> geometry_bdrFB({5, 4, 3}, iganet::init::greville, options);
    iganet::UniformBSpline<real_t, 4, 2, 3, 4> geometry_bdrSE({5, 4, 7}, iganet::init::greville, options);
    ...
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::func, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::func, false>(std::get<side::etime-1>(xi))[0])));
    ...
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dx, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dx, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dx, false>(std::get<side::etime-1>(xi))[0])));
    ...
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dx^2, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dx^2, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dx^2, false>(std::get<side::etime-1>(xi))[0])));
    ...
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dy, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dy, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dy, false>(std::get<side::etime-1>(xi))[0])));
    ...
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dy^2, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dy^2, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dy^2, false>(std::get<side::etime-1>(xi))[0])));
    ...
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dz, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dz, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dz, false>(std::get<side::etime-1>(xi))[0])));
    ...
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dz^2, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dz^2, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dz^2, false>(std::get<side::etime-1>(xi))[0])));

    eval = functionspace.eval<functionspace::boundary, deriv::dt, false>(xi);

    EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(eval)[0]),
                             *(bspline_bdrNS.eval<deriv::dt, false>(std::get<side::north-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(eval)[0]),
                             *(bspline_bdrNS.eval<deriv::dt, false>(std::get<side::south-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(eval)[0]),
                             *(bspline_bdrEW.eval<deriv::dt, false>(std::get<side::east-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(eval)[0]),
                             *(bspline_bdrEW.eval<deriv::dt, false>(std::get<side::west-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::front-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dt, false>(std::get<side::front-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dt, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dt, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dt, false>(std::get<side::etime-1>(xi))[0])));

    eval = functionspace.eval<functionspace::boundary, deriv::dt^2, false>(xi);

    EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(eval)[0]),
                             *(bspline_bdrNS.eval<deriv::dt^2, false>(std::get<side::north-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(eval)[0]),
                             *(bspline_bdrNS.eval<deriv::dt^2, false>(std::get<side::south-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(eval)[0]),
                             *(bspline_bdrEW.eval<deriv::dt^2, false>(std::get<side::east-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(eval)[0]),
                             *(bspline_bdrEW.eval<deriv::dt^2, false>(std::get<side::west-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::front-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dt^2, false>(std::get<side::front-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dt^2, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dt^2, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dt^2, false>(std::get<side::etime-1>(xi))[0])));
    ...
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dx+deriv::dy, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dx+deriv::dy, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dx+deriv::dy, false>(std::get<side::etime-1>(xi))[0])));
    ...
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dx+deriv::dz, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dx+deriv::dz, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dx+deriv::dz, false>(std::get<side::etime-1>(xi))[0])));

    eval = functionspace.eval<functionspace::boundary, deriv::dx+deriv::dt, false>(xi);

    EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(eval)[0]),
                             *(bspline_bdrNS.eval<deriv::dx+deriv::dt, false>(std::get<side::north-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(eval)[0]),
                             *(bspline_bdrNS.eval<deriv::dx+deriv::dt, false>(std::get<side::south-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(eval)[0]),
                             *(bspline_bdrEW.eval<deriv::dx+deriv::dt, false>(std::get<side::east-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(eval)[0]),
                             *(bspline_bdrEW.eval<deriv::dx+deriv::dt, false>(std::get<side::west-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::front-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dx+deriv::dt, false>(std::get<side::front-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dx+deriv::dt, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dx+deriv::dt, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dx+deriv::dt, false>(std::get<side::etime-1>(xi))[0])));
    ...
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dy+deriv::dz, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dy+deriv::dz, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dy+deriv::dz, false>(std::get<side::etime-1>(xi))[0])));

    eval = functionspace.eval<functionspace::boundary, deriv::dy+deriv::dt, false>(xi);

    EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(eval)[0]),
                             *(bspline_bdrNS.eval<deriv::dy+deriv::dt, false>(std::get<side::north-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(eval)[0]),
                             *(bspline_bdrNS.eval<deriv::dy+deriv::dt, false>(std::get<side::south-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(eval)[0]),
                             *(bspline_bdrEW.eval<deriv::dy+deriv::dt, false>(std::get<side::east-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(eval)[0]),
                             *(bspline_bdrEW.eval<deriv::dy+deriv::dt, false>(std::get<side::west-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::front-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dy+deriv::dt, false>(std::get<side::front-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dy+deriv::dt, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dy+deriv::dt, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dy+deriv::dt, false>(std::get<side::etime-1>(xi))[0])));

    eval = functionspace.eval<functionspace::boundary, deriv::dz+deriv::dt, false>(xi);

    EXPECT_TRUE(torch::equal(*(std::get<side::north-1>(eval)[0]),
                             *(bspline_bdrNS.eval<deriv::dz+deriv::dt, false>(std::get<side::north-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::south-1>(eval)[0]),
                             *(bspline_bdrNS.eval<deriv::dz+deriv::dt, false>(std::get<side::south-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::east-1>(eval)[0]),
                             *(bspline_bdrEW.eval<deriv::dz+deriv::dt, false>(std::get<side::east-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::west-1>(eval)[0]),
                             *(bspline_bdrEW.eval<deriv::dz+deriv::dt, false>(std::get<side::west-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::front-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dz+deriv::dt, false>(std::get<side::front-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(eval)[0]),
                             *(bspline_bdrFB.eval<deriv::dz+deriv::dt, false>(std::get<side::back-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dz+deriv::dt, false>(std::get<side::stime-1>(xi))[0])));
    EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(eval)[0]),
                             *(bspline_bdrSE.eval<deriv::dz+deriv::dt, false>(std::get<side::etime-1>(xi))[0])));
    ...
    basfunc = functionspace.template eval_basfunc<functionspace::boundary, deriv::dt, false>(xi, knot_indices);
    eval    = functionspace.template eval_from_precomputed<functionspace::boundary>(basfunc, coeff_indices, numel(xi), sizes(xi));
    ...
    basfunc = functionspace.template eval_basfunc<functionspace::boundary, deriv::dt^2, false>(xi, knot_indices);
    eval    = functionspace.template eval_from_precomputed<functionspace::boundary>(basfunc, coeff_indices, numel(xi), sizes(xi));
    ...
    basfunc = functionspace.template eval_basfunc<functionspace::boundary, deriv::dx+deriv::dt, false>(xi, knot_indices);
    eval    = functionspace.template eval_from_precomputed<functionspace::boundary>(basfunc, coeff_indices, numel(xi), sizes(xi));
    ...
    basfunc = functionspace.template eval_basfunc<functionspace::boundary, deriv::dy+deriv::dt, false>(xi, knot_indices);
    eval    = functionspace.template eval_from_precomputed<functionspace::boundary>(basfunc, coeff_indices, numel(xi), sizes(xi));
    ...
    basfunc = functionspace.template eval_basfunc<functionspace::boundary, deriv::dz+deriv::dt, false>(xi, knot_indices);
    eval    = functionspace.template eval_from_precomputed<functionspace::boundary>(basfunc, coeff_indices, numel(xi), sizes(xi));
    ...
    for (std::size_t i=0; i<std::get<0>(grad).entries(); ++i) {
      ...
      EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(grad)[i]),
                               *(bspline_bdrFB.grad<false>(std::get<side::back-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(grad)[i]),
                               *(bspline_bdrSE.grad<false>(std::get<side::stime-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(grad)[i]),
                               *(bspline_bdrSE.grad<false>(std::get<side::etime-1>(xi))[i])));
    }

    /// Evaluation of gradient (in physical domain)
    auto igrad = functionspace.template igrad<functionspace::boundary, false>(S4_geometry, xi);
    ...
    for (std::size_t i=0; i<std::get<0>(igrad).entries(); ++i) {
      ...
      EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(igrad)[i]),
                               *(bspline_bdrFB.igrad<false>(geometry_bdrFB, std::get<side::back-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(igrad)[i]),
                               *(bspline_bdrSE.igrad<false>(geometry_bdrSE, std::get<side::stime-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(igrad)[i]),
                               *(bspline_bdrSE.igrad<false>(geometry_bdrSE, std::get<side::etime-1>(xi))[i])));
    }
    ...
    for (std::size_t i=0; i<std::get<0>(jac).entries(); ++i) {
      ...
      EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(jac)[i]),
                               *(bspline_bdrFB.jac<false>(std::get<side::back-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(jac)[i]),
                               *(bspline_bdrSE.jac<false>(std::get<side::stime-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(jac)[i]),
                               *(bspline_bdrSE.jac<false>(std::get<side::etime-1>(xi))[i])));
    }

    /// Evaluation of Jacobian (in physical domain)
    auto ijac = functionspace.template ijac<functionspace::boundary, false>(S4_geometry, xi);
    ...
    for (std::size_t i=0; i<std::get<0>(ijac).entries(); ++i) {
      ...
      EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(ijac)[i]),
                               *(bspline_bdrFB.ijac<false>(geometry_bdrFB, std::get<side::back-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(ijac)[i]),
                               *(bspline_bdrSE.ijac<false>(geometry_bdrSE, std::get<side::stime-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(ijac)[i]),
                               *(bspline_bdrSE.ijac<false>(geometry_bdrSE, std::get<side::etime-1>(xi))[i])));
    }
    ...
    for (std::size_t i=0; i<std::get<0>(hess).entries(); ++i) {
      ...
      EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(hess)[i]),
                               *(bspline_bdrFB.hess<false>(std::get<side::back-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(hess)[i]),
                               *(bspline_bdrSE.hess<false>(std::get<side::stime-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(hess)[i]),
                               *(bspline_bdrSE.hess<false>(std::get<side::etime-1>(xi))[i])));
    }

    /// Evaluation of Hessian (in physical domain)
    auto ihess = functionspace.template ihess<functionspace::boundary>(S4_geometry, xi);
    ...
    for (std::size_t i=0; i<std::get<0>(ihess).entries(); ++i) {
      ...
      EXPECT_TRUE(torch::equal(*(std::get<side::back-1>(ihess)[i]),
                               *(bspline_bdrFB.ihess<false>(geometry_bdrFB, std::get<side::back-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::stime-1>(ihess)[i]),
                               *(bspline_bdrSE.ihess<false>(geometry_bdrSE, std::get<side::stime-1>(xi))[i])));
      EXPECT_TRUE(torch::equal(*(std::get<side::etime-1>(ihess)[i]),
                               *(bspline_bdrSE.ihess<false>(geometry_bdrSE, std::get<side::etime-1>(xi))[i])));
    }
  }
}
...
TEST_F(FunctionSpaceTest, RT1_geoDim1_degrees2)
{
  ...
  iganet::NonUniformBSpline<real_t, 1, 3> geometry0({5+1}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 1, 2> geometry1({5},   iganet::init::greville, options);

  { // Interior

    auto xi_ = iganet::utils::to_tensorArray<real_t>({0.0, 0.1, 0.2, 0.5, 0.75, 0.9, 1.0} /* u */, options);
    auto xi  = std::tuple{xi_, xi_};

    // Evaluation
    auto eval = functionspace.eval<functionspace::interior, deriv::func, false>(xi);
    ...
    eval = functionspace.eval<functionspace::interior, deriv::dx^2, false>(xi);
    ...
    auto basfunc = functionspace.template eval_basfunc<functionspace::interior, deriv::func, false>(xi, knot_indices);
    eval         = functionspace.eval_from_precomputed<functionspace::interior>(basfunc, coeff_indices, numel(xi), sizes(xi));
    ...
    /// Evaluation of gradient (in physical domain)
    auto igrad = functionspace.template igrad<functionspace::interior, false>(RT1_geometry, xi);
    ...
    /// Evaluation of Jacobian (in physical domain)
    auto ijac = functionspace.template ijac<functionspace::interior, false>(RT1_geometry, xi);
    ...
    /// Evaluation of Hessian (in physical domain)
    auto ihess = functionspace.template ihess<functionspace::interior, false>(RT1_geometry, xi);
...
TEST_F(FunctionSpaceTest, RT2_geoDim1_degrees23)
{
  ...
  iganet::NonUniformBSpline<real_t, 1, 2, 3>  bspline2({5, 6},   iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 2, 3, 3> geometry0({5+1, 6}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 2, 2, 4> geometry1({5, 6+1}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 2, 2, 3> geometry2({5, 6},   iganet::init::greville, options);

  { // Interior

    auto xi_ = iganet::utils::to_tensorArray<real_t>({0.0, 0.1, 0.2, 0.5, 0.75, 0.9, 1.0} /* u */,
                                                     {1.0, 0.2, 0.1, 0.5, 0.9, 0.75, 0.0} /* v */, options);
    ...
    eval = functionspace.eval<functionspace::interior, deriv::dy^2, false>(xi);
    ...
    eval = functionspace.eval<functionspace::interior, deriv::dx+deriv::dy, false>(xi);
    ...
    /// Evaluation of gradient (in physical domain)
    auto igrad = functionspace.template igrad<functionspace::interior, false>(RT2_geometry, xi);
    auto igrad_ref0 = bspline0.igrad(geometry0, xi_);
    auto igrad_ref1 = bspline1.igrad(geometry1, xi_);
    ...
    /// Evaluation of Jacobian (in physical domain)
    auto ijac = functionspace.template ijac<functionspace::interior, false>(RT2_geometry, xi);
    auto ijac_ref0 = bspline0.ijac(geometry0, xi_);
    auto ijac_ref1 = bspline1.ijac(geometry1, xi_);
    ...
    /// Evaluation of Hessian (in physical domain)
    auto ihess = functionspace.template ihess<functionspace::interior, false>(RT2_geometry, xi);
    auto ihess_ref0 = bspline0.ihess(geometry0, xi_);
    auto ihess_ref1 = bspline1.ihess(geometry1, xi_);
...
TEST_F(FunctionSpaceTest, RT3_geoDim1_degrees234)
{
  ...
  iganet::NonUniformBSpline<real_t, 1, 2, 3, 5>  bspline2({5, 6, 7+1}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 1, 2, 3, 4>  bspline3({5, 6, 7},   iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 3, 3, 3, 4> geometry0({5+1, 6, 7}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 3, 2, 4, 4> geometry1({5, 6+1, 7}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 3, 2, 3, 5> geometry2({5, 6, 7+1}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 3, 2, 3, 4> geometry3({5, 6, 7},   iganet::init::greville, options);
  ...
  { // Interior
    ...
    eval = functionspace.eval<functionspace::interior, deriv::dz^2, false>(xi);
    ...
    eval = functionspace.eval<functionspace::interior, deriv::dx+deriv::dz, false>(xi);
    ...
    eval = functionspace.eval<functionspace::interior, deriv::dy+deriv::dz, false>(xi);
    ...
    /// Evaluation of gradient (in physical domain)
    auto igrad = functionspace.template igrad<functionspace::interior, false>(RT3_geometry, xi);
    auto igrad_ref0 = bspline0.igrad(geometry0, xi_);
    auto igrad_ref1 = bspline1.igrad(geometry1, xi_);
    auto igrad_ref2 = bspline2.igrad(geometry2, xi_);
    ...
    /// Evaluation of Jacobian (in physical domain)
    auto ijac = functionspace.template ijac<functionspace::interior, false>(RT3_geometry, xi);
    auto ijac_ref0 = bspline0.ijac(geometry0, xi_);
    auto ijac_ref1 = bspline1.ijac(geometry1, xi_);
    auto ijac_ref2 = bspline2.ijac(geometry2, xi_);
    ...
    /// Evaluation of Hessian (in physical domain)
    auto ihess = functionspace.template ihess<functionspace::interior, false>(RT3_geometry, xi);
    auto ihess_ref0 = bspline0.ihess(geometry0, xi_);
    auto ihess_ref1 = bspline1.ihess(geometry1, xi_);
    auto ihess_ref2 = bspline2.ihess(geometry2, xi_);
...
TEST_F(FunctionSpaceTest, RT4_geoDim1_degrees2341)
{
  ...
  iganet::NonUniformBSpline<real_t, 1, 2, 3, 5, 1>  bspline2({5, 6, 7+1, 4}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 1, 2, 3, 4, 2>  bspline3({5, 6, 7, 4+1}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 1, 2, 3, 4, 1>  bspline4({5, 6, 7, 4},   iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 4, 3, 3, 4, 1> geometry0({5+1, 6, 7, 4}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 4, 2, 4, 4, 1> geometry1({5, 6+1, 7, 4}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 4, 2, 3, 5, 1> geometry2({5, 6, 7+1, 4}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 4, 2, 3, 4, 2> geometry3({5, 6, 7, 4+1}, iganet::init::greville, options);
  iganet::NonUniformBSpline<real_t, 4, 2, 3, 4, 1> geometry4({5, 6, 7, 4},   iganet::init::greville, options);
  ...
  { // Interior
    ...
    eval = functionspace.eval<functionspace::interior, deriv::dt^2, false>(xi);
    ...
    eval = functionspace.eval<functionspace::interior, deriv::dx+deriv::dt, false>(xi);
    ...
    eval = functionspace.eval<functionspace::interior, deriv::dy+deriv::dt, false>(xi);
    ...
    eval = functionspace.eval<functionspace::interior, deriv::dz+deriv::dt, false>(xi);
    ...
    /// Evaluation of gradient (in physical domain)
    auto igrad = functionspace.template igrad<functionspace::interior, false>(RT4_geometry, xi);
    auto igrad_ref0 = bspline0.igrad(geometry0, xi_);
    auto igrad_ref1 = bspline1.igrad(geometry1, xi_);
    auto igrad_ref2 = bspline2.igrad(geometry2, xi_);
    auto igrad_ref3 = bspline3.igrad(geometry3, xi_);
    ...
    /// Evaluation of Jacobian (in physical domain)
    auto ijac = functionspace.template ijac<functionspace::interior, false>(RT4_geometry, xi);
    auto ijac_ref0 = bspline0.ijac(geometry0, xi_);
    auto ijac_ref1 = bspline1.ijac(geometry1, xi_);
    auto ijac_ref2 = bspline2.ijac(geometry2, xi_);
    auto ijac_ref3 = bspline3.ijac(geometry3, xi_);
    ...
    /// Evaluation of Hessian (in physical domain)
    auto ihess = functionspace.template ihess<functionspace::interior, false>(RT4_geometry, xi);
    auto ihess_ref0 = bspline0.ihess(geometry0, xi_);
    auto ihess_ref1 = bspline1.ihess(geometry1, xi_);
    auto ihess_ref2 = bspline2.ihess(geometry2, xi_);
    auto ihess_ref3 = bspline3.ihess(geometry3, xi_);
