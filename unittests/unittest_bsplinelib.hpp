...
template <iganet::deriv deriv, bool memory_optimized, bool precompute,
    typename Spline, typename BSplineLibNurbs, typename TensorArray_t>
    void test_nurbs_eval(const Spline& bspline,
        BSplineLibNurbs& bsplinelib_bspline,
        const TensorArray_t& xi,
        typename Spline::value_type tol = 1e-12) {
    ...
    // B-spline evaluation
    using Derivative = typename BSplineLibNurbs::ParameterSpace_::Derivative_;
    using ScalarDerivative = typename Derivative::value_type;
    using Coordinate = typename BSplineLibNurbs::Coordinate_;
    using ScalarCoordinate = typename Coordinate::value_type;
...
template <bool memory_optimized, bool precompute, typename Spline,
    typename TensorArray_t>
    void test_nurbs_grad(const Spline& bspline, const TensorArray_t& xi,
        typename Spline::value_type tol = 1e-12) {
    iganet::utils::BlockTensor<torch::Tensor, 1, Spline::parDim()>
        bspline_grad_val;
...
template <bool memory_optimized, bool precompute, typename Geometry_t,
    typename Spline, typename TensorArray_t>
    void test_nurbs_igrad(const Geometry_t& geometry, const Spline& bspline,
        const TensorArray_t& xi,
        typename Spline::value_type tol = 1e-12) {
    iganet::utils::BlockTensor<torch::Tensor, 1, Spline::parDim()>
        bspline_igrad_val;
...
void test_nurbs_eval(const Geometry_t& geometry, const Spline& bspline,
    ...
    typename Spline::value_type tol = 1e-12) {
    // Create B-Spline
    bsplinelib::splines::Nurbs<bspline.parDim()> bsplinelib_bspline = to_bsplinelib_nurbs(bspline);
    // Evaluate function and derivatives (non-memory optimized)
    ...
    if constexpr (Spline::parDim() == 1) {
        test_nurbs_eval<iganet::deriv::dx, false, false>(
            bspline, bsplinelib_bspline, xi, tol);
        /*  test_nurbs_eval<iganet::deriv::dx ^ 2, false, false>( //TODO: Check if it throws assertation error if different one.
              bspline, bsplinelib_bspline, xi, tol);
          test_nurbs_eval<iganet::deriv::dx ^ 3, false, false>(
              bspline, bsplinelib_bspline, xi, tol);
          test_nurbs_eval<iganet::deriv::dx ^ 4, false, false>(
              bspline, bsplinelib_bspline, xi, tol); */  //TODO: Check what it does if degree to low??
    }
    if constexpr (Spline::parDim() == 2) {
        test_nurbs_eval<iganet::deriv::dx, false, false>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dy, false, false>(
            bspline, bsplinelib_bspline, xi, tol);
        /* test_nurbs_eval<iganet::deriv::dy ^ 2, false, false>(
             bspline, bsplinelib_bspline, xi, tol);
         test_nurbs_eval<iganet::deriv::dy ^ 3, false, false>(
             bspline, bsplinelib_bspline, xi, tol);
         test_nurbs_eval<iganet::deriv::dy ^ 4, false, false>(
             bspline, bsplinelib_bspline, xi, tol);*/
    }

    if constexpr (Spline::parDim() == 3) {
        test_nurbs_eval<iganet::deriv::dz, false, false>(
            bspline, bsplinelib_bspline, xi, tol);
        /*    test_nurbs_eval<iganet::deriv::dz ^ 2, false, false>(
                bspline, bsplinelib_bspline, xi, tol);
            test_nurbs_eval<iganet::deriv::dz ^ 3, false, false>(
                bspline, bsplinelib_bspline, xi, tol);
   ...
   /* test_nurbs_eval<iganet::deriv::func, true, false>(
    ...
    if constexpr (Spline::parDim() == 1) {
     ...
     /*   test_nurbs_eval<iganet::deriv::dx ^ 2, true, false>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dx ^ 3, true, false>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dx ^ 4, true, false>(
            bspline, bsplinelib_bspline, xi, tol);
    }

    if constexpr (Spline::parDim() == 2) {
        test_nurbs_eval<iganet::deriv::dy, true, false>(
            bspline, bsplinelib_bspline, xi, tol);
       /* test_nurbs_eval<iganet::deriv::dy ^ 2, true, false>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dy ^ 3, true, false>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dy ^ 4, true, false>(
            bspline, bsplinelib_bspline, xi, tol);
    }

    if constexpr (Spline::parDim() == 3) {
        test_nurbs_eval<iganet::deriv::dz, true, false>(
            bspline, bsplinelib_bspline, xi, tol);
     /*   test_nurbs_eval<iganet::deriv::dz ^ 2, true, false>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dz ^ 3, true, false>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dz ^ 4, true, false>(
            bspline, bsplinelib_bspline, xi, tol);
    }
   ...
   /* test_nurbs_eval<iganet::deriv::func, false, true>(
    ...
    if constexpr (Spline::parDim() == 2) {
        ...
        test_nurbs_eval<iganet::deriv::dy ^ 3, false, true>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dy ^ 4, false, true>(
            bspline, bsplinelib_bspline, xi, tol);
    }
    ...
    if constexpr (Spline::parDim() == 3) {
        ...
        test_nurbs_eval<iganet::deriv::dz ^ 3, false, true>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dz ^ 4, false, true>(
            bspline, bsplinelib_bspline, xi, tol);
    }
    ...
    if constexpr (Spline::parDim() == 4) {
        ...
        test_nurbs_eval<iganet::deriv::dt ^ 2, false, true>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dt ^ 3, false, true>(
            bspline, bsplinelib_bspline, xi, tol);
   ...
   // Evaluate function and derivatives from precomputed data (memory optimized)
    ...
    if constexpr (Spline::parDim() == 1) {
        ...
        test_nurbs_eval<iganet::deriv::dx ^ 3, true, true>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dx ^ 4, true, true>(
            bspline, bsplinelib_bspline, xi, tol);
    }
    ...
    if constexpr (Spline::parDim() == 2) {
        ...
        test_nurbs_eval<iganet::deriv::dy ^ 3, true, true>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dy ^ 4, true, true>(
            bspline, bsplinelib_bspline, xi, tol);
    }
    ...
    if constexpr (Spline::parDim() == 3) {
        ...
        test_nurbs_eval<iganet::deriv::dz ^ 3, true, true>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dz ^ 4, true, true>(
            bspline, bsplinelib_bspline, xi, tol);
    }
    ...
    if constexpr (Spline::parDim() == 4) {
        ...
        test_nurbs_eval<iganet::deriv::dt ^ 2, true, true>(
            bspline, bsplinelib_bspline, xi, tol);
        test_nurbs_eval<iganet::deriv::dt ^ 3, true, true>(
            bspline, bsplinelib_bspline, xi, tol);
   ...
   // Evaluate gradients
    if constexpr (Spline::geoDim() == 1) {
        ...
        test_nurbs_grad<true, false>(bspline, xi, tol);
        test_nurbs_grad<true, true>(bspline, xi, tol);

        test_nurbs_igrad<false, false>(geometry, bspline, xi, tol);
  ...
  //  test_nurbs_ijac<true, false>(geometry, bspline, xi, tol);
    ...
    /* if constexpr (Spline::geoDim() == 1) {
         test_nurbs_hess<false, false>(bspline, xi, tol);
         test_nurbs_hess<false, true>(bspline, xi, tol);
         test_nurbs_hess<true, false>(bspline, xi, tol);
         test_nurbs_hess<true, true>(bspline, xi, tol);

         test_nurbs_ihess<false, false>(geometry, bspline, xi, tol);
         test_nurbs_ihess<false, true>(geometry, bspline, xi, tol);
         test_nurbs_ihess<true, false>(geometry, bspline, xi, tol);
         test_nurbs_ihess<true, true>(geometry, bspline, xi, tol);
     }*/
}